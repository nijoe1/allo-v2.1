...
import {SablierAllocationExtension} from "strategies/extensions/streaming/sablier/SablierAllocationExtension.sol";
// Internal Libraries
...
/// by the pool manager.
contract DonationVotingOffchainWithSablier is BaseStrategy, RecipientsExtension, SablierAllocationExtension {
    ...
    function initialize(uint256 _poolId, bytes memory _data) external virtual override {
        ...
        ) = abi.decode(_data, (RecipientInitializeData, uint64, uint64, uint64, address[], bool, address, bool, uint40));

        withdrawalCooldown = _withdrawalCooldown;

        __BaseStrategy_init(_poolId);
        __RecipientsExtension_init(_recipientExtensionInitializeData);
        __SablierAllocationExtension_init(_allowedTokens,_lockupLinear, _streamingDuration, _cancelableStream, _allocationStartTime, _allocationEndTime, _isUsingAllocationMetadata);
        ...
        onlyActiveAllocation
    {
        ...
        for (uint256 i; i < __recipients.length; i++) {
            ...
            if (_tokens[i] == Transfer.NATIVE) {
                revert OnlyERC20_ALLOWED_TOKENS();
            } else {
        ...
        onlyAfterAllocation
    {
        for (uint256 i; i < _recipientIds.length; i++) {
            ...
            uint256 _stramID = _startSingleStreamDistribution(_recipientAddress, _pool.token, _amount);
            recipientStreams[_recipientId] = _stramID;
